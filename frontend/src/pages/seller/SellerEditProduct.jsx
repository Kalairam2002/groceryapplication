...
const SellerEditProduct = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        // Fetch variants for existing subcategory
        if (p.subcategory) {
          const { data: varData } = await axios.get(
            `${API}/api/variant/bySubCategory/${p.subcategory}`
          );
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API}/api/variant/bySubCategory/${form.subcategory}`)
      .then((res) => {
        if (res.data.success) {
          setVariants(res.data.variants || []);
          // Reset variant if subcategory changed
          setForm((prev) => ({
            ...prev,
            variant: prev.variant && res.data.variants.some(v => v._id === prev.variant)
              ? prev.variant
              : "",
          }));
        }
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-row mt-2">
            <select
              ...
              value={form.variant}
              onChange={handleChange}
              disabled={!form.subcategory || variants.length === 0}
              required
            >
