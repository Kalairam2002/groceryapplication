...
const Varientdatadisplay = ({ id }) => {
  ...
  const { data: variantsdata, isLoading: issubCategoryLoading } = useQuery({
    
    
  queryKey: ["variantskey",id],
  queryFn: async () => {
    const res = await axios.get(
      `${process.env.REACT_APP_API_URL}/api/test/getveriantdata/${id}`
  ...
  const displayedProducts = id
    ? products.filter((p) => String(p.category) === id)
        ...
        <div className="row">
          ...
          <div className="col-lg-12">
            <div className="flex-between gap-16 flex-wrap mb-40">
              <span className="text-gray-900"><b>List of variants</b></span>
            ...
            <div className={`list-grid-wrapper ${grid && "list-view"}`}>
              <div className="row gy-4 g-12">
                {!issubCategoryLoading && variantsdata ? (
                  variantsdata.map((product) => (
                    <div
                      key={product._id}
                    ...
                    >
                      <div className="product-card px-8 py-16 border border-gray-100 hover-border-main-600 rounded-16 position-relative transition-2">
                        ...
                        <Link to={`/Productlist/${product._id}`} className="product-card__thumb flex-center">
                          <img
...
export default Varientdatadisplay;
