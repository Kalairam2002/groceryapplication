...
export const forgotAdminPassword = async (req, res) => {
  try {
    ...
    const html = `<p>Hello ${admin.name},</p>
                  <p>Click below to reset your password:</p>
                  <a href="${resetLink}">${resetLink}</a>
                  <p>This link expires in 1 hour.</p>`;

    await sendEmail(admin.email, "Admin Password Reset", html);

    res.json({ success: true, message: "Password reset link sent to admin email.", token: resetToken });
  } catch (error) {
    console.error("Forgot admin password error:", error.message);
    res.status(500).json({ success: false, message: "Server error" });
  }
};
...
export const resetAdminPassword = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: "Admin password reset successful. Please log in with your new password." });
  } catch (error) {
