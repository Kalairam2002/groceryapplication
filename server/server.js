...
app.post("/api/create-order", async (req, res) => {
  try {
    ...
    if (!CF_APP_ID || !CF_SECRET_KEY || !CF_ENV) {
      console.error("Cashfree environment variables are missing");
      return res.status(500).json({ error: "Cashfree configuration is missing" });
    }
    ...
    const payload = {
      ...
      customer_details: {
        ...
        customer_phone: user?.phone || user?.phoneNumber || "9999999999"
      },
      order_meta: {
  ...
  try {
    ...
    const enrichedProducts = await Promise.all(
      cart.map(async (item) => {
        ...
        if (!matchedVariant) {
          throw new Error(`${prod.name} — variant not found`);
        }

        //  Check stock — real total is the sum of batch stock, not the
        //  legacy flat field on the product, which is stale/unused for
        //  products that have been migrated to batch-tracked stock
        const totalStock = matchedVariant.getTotalStock();
        if (totalStock < purchaseQty) {
        ...
        if (matchedVariant.batches && matchedVariant.batches.length > 0) {
          let remaining = purchaseQty;
          const sortedBatches = [...matchedVariant.batches].sort((a, b) => {
          ...
          matchedVariant.stock = Number(matchedVariant.stock) - purchaseQty;
        }
        await prod.save();
        ...
        return {
          ...
          price: resolvedPrice,
          seller: prod.seller?._id || null,
          sellerName: prod.seller?.name || "N/A"
        };
      })
    );
    ...
    try {
      ...
      if (dbUser) {
        ...
        await sendEmail(dbUser.email, "maligaijaman - Order Confirmation", emailHTML);
        console.log("Order confirmation email sent to", dbUser.email);
      }
    } catch (emailErr) {
      console.error("[verifyPayment] Email failed:", emailErr.message);
    }
    ...
    try {
      ...
      if (dbUser?.phoneNumber) {
        ...
        await sendWhatsAppMessage(phone, dbUser.username, orderId, amount);
      }
    } catch (whatsappErr) {
      console.error("[verifyPayment] WhatsApp failed:", whatsappErr.message);
    }
